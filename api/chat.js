...
function makeMatch(row) {
  return { date: row[0], group: row[1], home: row[2], away: row[3], hg: row[4], ag: row[5], homeCrest: row[6] || "", awayCrest: row[7] || "" };
}

function makeUpcoming(row) {
  return { date: row[0], group: row[1], home: row[2], away: row[3], status: row[4] || "À venir", homeCrest: row[5] || "", awayCrest: row[6] || "" };
}
...
async function loadContext(leagueSlug) {
  ...
  if (league.slug === "trophee-des-champions") {
    ...
    try {
      ...
      const RESULTS = recent.map(f => [f.date, f.round, f.home, f.away, f.hg, f.ag, f.homeCrest, f.awayCrest]);
  ...
  try {
    ...
    const RESULTS = recent.map(f => [f.date, f.round, f.home, f.away, f.hg, f.ag, f.homeCrest, f.awayCrest]);
    const UPCOMING = upcoming.map(f => [f.date, f.round, f.home, f.away, statusToLabel(f.status), f.homeCrest, f.awayCrest]);
